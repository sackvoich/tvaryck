    ...
    while True:
        ...
        if source_face is not None and source_mask is not None:
            ...
            for face_landmarks in face_landmarks_list:
                ...
                # Проверяем, находятся ли хотя бы одна точка лица в пределах кадра
                if not (0 <= np.min(target_points[:, 0]) < frame.shape[1] and
                    ...
                    continue  # Пропускаем текущее лицо, если оно частично или полностью вне кадра
                ...
                # Коррекция center_point, чтобы убедиться, что он находится внутри границ изображения
                ...
                # Предполагаем, что center_point теперь всегда валидный
